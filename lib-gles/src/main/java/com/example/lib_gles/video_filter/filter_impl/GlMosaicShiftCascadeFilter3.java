...
/**
 * Minimal composite filter:
 * 1) mosaic keyframes
 * 2) simple zoom events
 * 3) ScaleBlur-style shake events
 *
 * No default timeline is installed. Configure only the pieces you need.
 */
public class GlMosaicShiftCascadeFilter3 extends GlFilter {
    ...
    public GlMosaicShiftCascadeFilter3() {
    ...
    public GlMosaicShiftCascadeFilter3 clearMosaicKeyframes() {
        mosaicKeyframes.clear();
        return this;
    }

    public GlMosaicShiftCascadeFilter3 addMosaicKeyframe(float timeMs, float blockSize) {
        mosaicKeyframes.add(new MosaicKeyframe(Math.max(0f, timeMs), Math.max(0f, blockSize)));
        sortMosaicKeyframes();
        return this;
    }

    public GlMosaicShiftCascadeFilter3 setMosaicMaxBlockSize(float mosaicMaxBlockSize) {
        this.mosaicMaxBlockSize = Math.max(1f, mosaicMaxBlockSize);
        return this;
    }

    public GlMosaicShiftCascadeFilter3 addMosaicLevelKeyframe(float timeMs, float level) {
        return addMosaicKeyframe(timeMs, clamp01(level) * mosaicMaxBlockSize);
    }

    public GlMosaicShiftCascadeFilter3 clearZoomEvents() {
        zoomEvents.clear();
        return this;
    }

    public GlMosaicShiftCascadeFilter3 addZoomEvent(float startMs, float endMs, float fromScale, float toScale, int ease) {
    ...
    public GlMosaicShiftCascadeFilter3 clearShakeEvents() {
        shakeEvents.clear();
        return this;
    }

    public GlMosaicShiftCascadeFilter3 addPulseShakeEvent(float startMs, float endMs, float ampX, float ampY) {
        return addShakeEvent(startMs, endMs, ampX, ampY, SHAKE_MODE_PULSE, 1);
    }

    public GlMosaicShiftCascadeFilter3 addOscillatingShakeEvent(float startMs, float endMs, float ampX, float ampY, int cycles) {
        return addShakeEvent(startMs, endMs, ampX, ampY, SHAKE_MODE_OSCILLATE, cycles);
    }

    public GlMosaicShiftCascadeFilter3 addShakeEvent(float startMs, float endMs, float ampX, float ampY, int mode, int cycles) {
        float s = Math.max(0f, startMs);
        float e = Math.max(s + 1f, endMs);
    ...
    public GlMosaicShiftCascadeFilter3 setShakeScalePadding(float shakeScalePadding) {
        this.shakeScalePadding = Math.max(0f, shakeScalePadding);
        return this;
    }

    public GlMosaicShiftCascadeFilter3 setMaxShakeScale(float maxShakeScale) {
        this.maxShakeScale = Math.max(1.0f, maxShakeScale);
        return this;
    }

    public GlMosaicShiftCascadeFilter3 setMaxShakeStretch(float maxShakeStretchX) {
        this.maxShakeStretchX = Math.max(1.0f, maxShakeStretchX);
        return this;
    }

    public GlMosaicShiftCascadeFilter3 setMaxShakeStretch(float maxShakeStretchX, float maxShakeStretchY) {
        this.maxShakeStretchX = Math.max(1.0f, maxShakeStretchX);
        this.maxShakeStretchY = Math.max(1.0f, maxShakeStretchY);
        return this;
    }

    public GlMosaicShiftCascadeFilter3 setMaxShakeBlur(float maxShakeBlur) {
        this.maxShakeBlur = clamp(maxShakeBlur, 0.0f, 0.12f);
        return this;
    }

    public GlMosaicShiftCascadeFilter3 setMaxShakeSampleScale(float maxShakeSampleScale) {
